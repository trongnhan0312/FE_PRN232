...
export const updateDonation = async (id, model) => {
    try {
        ...
        const res = await axios.put(
            `${API_ENDPOINT.DONATION.UPDATE.replace(
                "{id}",
                id
            )}?${params.toString()}`
        );
...
export const getDoctorPagination = async ({
    ...
    try {
        ...
        const res = await axios.post(
            `${
                API_ENDPOINT.EMPLOYEE.GET_DOCTORS_PAGINATION
            }?${params.toString()}`
        );
        return res.data;
    } catch (err) {
...
// Lấy danh sách lịch sử hiến máu của user (trả về mảng items, giống homepage)
export const getUserDonations = async (
    ...
    // Trả về đúng mảng items (hoặc [] nếu không có)
    return res.data.resultObj?.items || [];
};

// Lấy chi tiết 1 lần hiến máu của user (trả về object donation, giống homepage detail)

...
export const updateBloodUnit = async (id, model) => {
  try {
    console.log("🚀 [updateBloodUnit] Gửi payload:", model);

    const params = new URLSearchParams();

    if (model.bloodGroupId) params.append("BloodGroupId", model.bloodGroupId);
    if (model.bloodComponent)
      params.append("BloodComponent", model.bloodComponent);
    if (model.quantity) params.append("Quantity", model.quantity);
    if (model.expiryDate) params.append("ExpiryDate", model.expiryDate);
    ...
    console.log("✅ [updateBloodUnit] Thành công:", res.data);
    return res.data;
  } catch (err) {
    console.error("❌ [updateBloodUnit] Lỗi:", err);
    if (err.response?.data) {
      console.error("📋 [updateBloodUnit] Backend trả về:", err.response.data);
    }
    throw err;
  }
};
...
export const updateBloodRequest = async (id, model) => {
  try {
    ...
    // 👇 Thêm từng field cẩn thận
    formData.append("RequestedById", model.RequestedById ?? "");
    formData.append("BloodGroupId", model.BloodGroupId ?? "");
    formData.append("Quantity", model.Quantity ?? 0);
    formData.append("QuantityFromStock", model.QuantityFromStock ?? 0);
    formData.append("Notes", model.Notes || "");
    formData.append("BloodComponent", model.BloodComponent || "");
    formData.append("IsEmergency", model.IsEmergency ? "true" : "false");
    formData.append("RequestSource", model.RequestSource || "");
    formData.append("Status", model.Status || "");
    formData.append("BloodUnitId", model.BloodUnitId ?? "");

    console.log("🚀 Payload FormData gửi lên:");
    ...
    console.log("✅ [updateBloodRequest] Response:", res.data);
...
export const getUserBlogPostById = async (id) => {
  const res = await axios.get(API_ENDPOINT.BLOGPOST.DETAIL.replace("{id}", id));
  return res.data.resultObj;
};

// ---------------- USER - BLOOD GROUP ------------------
export const getUserBloodGroups = async ({
  pageNumber = 1,
  pageSize = 20,
} = {}) => {
  const params = new URLSearchParams();
...
export const getUserBloodGroupById = async (id) => {
  const res = await axios.get(
    API_ENDPOINT.BLOOD_GROUP.DETAIL.replace("{id}", id)
  );
  return res.data.resultObj;
...
// ---------------- USER - BLOOD COMPATIBILITY ------------------
export const getUserBloodCompatibilities = async ({
  pageNumber = 1,
  pageSize = 20,
} = {}) => {
  const params = new URLSearchParams();
...
export const getUserBloodCompatibilityById = async (id) => {
  const res = await axios.get(
    API_ENDPOINT.BLOOD_COMPATIBILITY.DETAIL.replace("{id}", id)
  );
  return res.data.resultObj;
...
// ---------------- USER - DONATION (Lịch sử hiến máu của user) ------------------
...
export const getUserDonationById = async (id) => {
...
export const getUserDonorAvailableById = async (id) => {
  const res = await axios.get(
    API_ENDPOINT.DONOR_AVAILABILITY.DETAIL.replace("{id}", id)
  );
  return res.data.resultObj;
...
export const getUserProfile = async (id) => {
  try {
    const res = await axios.get(`${API_ENDPOINT.USER.PROFILE}${id}`);
    return res.data;
  } catch (err) {
...
export const updateUserProfile = async (data) => {
  try {
    ...
    if (data.bloodGroupId) formData.append("BloodGroupId", data.bloodGroupId);
    if (data.avatarFile) formData.append("AvatarUrl", data.avatarFile);
    const res = await axios.put(API_ENDPOINT.USER.UPDATE_PROFILE, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    return res.data;
  } catch (err) {
...
// ---------------- USER - BLOOD REQUEST (Create, GetAll, Detail, Delete) ------------------
export const createUserBloodRequest = async (data) => {
  // Không truyền BloodUnitId và số lượng kho, chỉ truyền các trường cần thiết cho user
  const formData = new FormData();
...
export const getUserBloodRequestById = async (id) => {
  const res = await axios.get(
    API_ENDPOINT.BLOOD_REQUEST.DETAIL.replace("{id}", id)
  );
  return res.data.resultObj;
...
export const deleteUserBloodRequest = async (id) => {
  const res = await axios.delete(
    API_ENDPOINT.BLOOD_REQUEST.DELETE.replace("{id}", id)
  );
  return res.data;
